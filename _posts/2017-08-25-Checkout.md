...
|**Relatórios transacionais**|Dentro do Backoffice, é possivel gerar relatorios transancionais que facilitam a administração de suas vendas: <br><br> **Relatório de Recorrências** <br> **Relatório de compradores**<br>**Extrato de vendas**<br>**Relatório de vendas**<br><br>|
...
O Checkout Cielo  possui uma lista de requisitos basicos para que o processo de integração seja bem sucedido.
...
![Fluxo de integração Checkout Cielo](http://fit-chair.cloudvent.net/Tutorial/public/images/Checkout/fluxocheckoutbasico.png)
...
![Ativando Modo de teste](http://fit-chair.cloudvent.net/Tutorial/public/images/Checkout/TM01.png)
...
|--------------|--------------|
|![Tarja vermelha - Backoffice](http://fit-chair.cloudvent.net/Tutorial/public/images/Checkout/TMBackoffice.png)|![Tarja vermelha - Transacional](http://fit-chair.cloudvent.net/Tutorial/public/images/Checkout/TMtransacional.PNG)|
...
![Integração Via API](http://fit-chair.cloudvent.net/Tutorial/public/images/Checkout/IntAPI.png)
### Criando o Carrinho


Na integração via API, a tela transacional é "montada" com bases em dados enviados que formam um **Carrinho de compras**.
Esses dados são separados nos sequintes "nós principais":
...
Devido ao seu fluxo de venda ser dividio em duas etapas, sendo a primeira, a criação da tela transacional e a segunda, a finalização do pagamento; O Checkout possui duas respostas para uma transação:

* **Response - Tela transacional** - É o Response retornado com dados para enviar o comprador para a tela transacional
* **Response - Transação Finalizada** - Contem dados sobre o resultado da transação, após o comprador clica em "Finalizar" na tela transacional. **É retornado apenas via Notificação**


> **Resultado/Status da transação:** Para obter o retorno do status da transação, é necessario definir uma URL de NOTIFICAÇÃO. Veja a sessão de notificação para maiores informações.
...
**Sucesso** - Em caso de sucesso, o response será o conteudo do Request mais o Link que direciona a tela transacional
...
Nos items a seguir, será explicado o comportamento de algumas das funcionalidades da integração via API. Essas funcionalides possuem regras especificas para utilização e não estão disponiveis na integração via Botão.
...
O Checkout Cielo permite que o lojista aplique descontos especificos tanto para o carrinho quanto para meios de pagamento.
Os descontos disponiveis no Checkout Cielo são:
...
|Percentual|Valor|
|----------|-----|
|![Percentual](http://fit-chair.cloudvent.net/Tutorial/public/images/Checkout/checkout-discount-percent.png)|![Valor](http://fit-chair.cloudvent.net/Tutorial/public/images/Checkout/checkout-discount-amount.png)|
...
|Tela transacional|
|-----------------|
|![Meios de pagamento](http://fit-chair.cloudvent.net/Tutorial/public/images/Checkout/checkout-discount-MP.PNG)|
...
|:-----------------------:|:------------------------------------------------------------------------------------------------------------------:|
| `FixedAmount`           | ![FixedAmount](http://fit-chair.cloudvent.net/Tutorial/public/images/Checkout/FixedAmount.PNG)                     |
| `Free`                  | ![Free](http://fit-chair.cloudvent.net/Tutorial/public/images/Checkout/Free.PNG)                                   |
| `WithoutShippingPickUp` | ![WithoutShippingPickUp](http://fit-chair.cloudvent.net/Tutorial/public/images/Checkout/WithoutShippingPickUp.PNG) |
| `WithoutShipping`       | ![WithoutShipping](http://fit-chair.cloudvent.net/Tutorial/public/images/Checkout/WithoutShippingPickUp.PNG)       |
| `Correios`              | ![Correios](http://fit-chair.cloudvent.net/Tutorial/public/images/Checkout/Correios.PNG)                           |

> **OBS:** As opções para multiplos fretes na categoria `Correios` devem ser selecionadas dentro do Backoffice Cielo.        
...
* **Frete sem Volume** - Utiliza a API dos correios, mas considera apenas o peso do carrinho como base de calculo para a entrega.
...
Esse tipo de integração é realizada via o cadastro de um conjunto de itens a ser vendido on backoffice do Checkout Cielo.

O gera um 3 tipos diferentes de métodos de acesso a **mesma tela transacional**:

| Método    |Nome| Descrição|
|-----------|:--:|----------|
|![Botão](http://fit-chair.cloudvent.net/Tutorial/public/images/Checkout/Botão.PNG)|**Botão**|É um código HTML que ao ser colado em um site, vai direcionar o comprador a tela transacional - Ideal para uso em **hotSites** ou **E-mail Marketing** |
|![QRCODE](http://fit-chair.cloudvent.net/Tutorial/public/images/Checkout/Qrcode.PNG)   |**QRCODE**|Código interpretavél por Smartphones e Tablets - Idela para uso em **Marketing impressos** ou **Digital**|
|`http://bit.ly/2tRkSxZ`|**LINK**|é um link compartilhavel, ideal para uso em **Redes Sociais** ou **Messengers Mobile**|
...
| Caracteristica | Explicação |
|:--------------:|------------|
|**Especifico**| Cada botão gerado serve somente para um determinado produto ou grupo de produtos.A quantidade e volume de produtos vendido é definido no cadastro do Botão, não sendo possivel altera a quantidade na tela transacional <BR><BR>**Exemplo:** Será necessario criar Um botão para vender 1 camisa. Se o comprador desejar 3 camisas, ele precisará usar o botão 2X ou O lojista deverá criar um botão com 2 camisas|
|**Numero do Pedido do Checkout**| O botão não permite o cadastro do numero de pedido do Lojista. Como será a Cielo a acionar o proprio Checkout, será gerado um numero de pedido (um `GUID`) unico. O Lojista receberá esse numero de pedido como link a venda realizada|
|**Criação de pedidos**|Um botão gera varios pedidos independentes, ou seja, não é possivel limitar a quantidade de pedidos gerados por um botão,QRCODE ou Link criado. O Botão é um método de chamadas à API Checkout. Cada vez que ele pe acionado, uma nova requisição é feita a API, criando assim um novo pedido| 


**Abaixo, o fluxo de pagamento via Botão:**

![Fluxo de integração Checkout Cielo Botão](http://fit-chair.cloudvent.net/Tutorial/public/images/Checkout/IntBT.png)
...
![Cadastro de Botão](http://fit-chair.cloudvent.net/Tutorial/public/images/Checkout/BTCadastro.PNG)


**Botão Cadastrado:**

![Cadastro de Botão](http://fit-chair.cloudvent.net/Tutorial/public/images/Checkout/BTCadastro2.PNG)
...
Abaixo é possivel ver como o cadastro de um botão gera os 3 métodos de para acesso a tela transacional.
...
| `N/A`             | Autenticado              | Transações autenticadas pelo banco - **Não são analisaveis pelo AF**                                            |
| `N/A`             | AF Não contratado        | Antifraude não habilitado no plano do lojista - **Não são analisaveis pelo AF**                                 |
...
| `N/A`             | Transação negada         | Venda a crédito foi negada - **Não são analisaveis pelo AF**                                                    |
...
O Checkout Cielo permite a utilização de Cartões de débito MasterCard e Visa. Esse meio de pagamento é liberado automaticamente junto a afiliação de Cielo, podendo ser utilizado inicialente com a integração Checkout.
...
**OBS:** Cartões não pertecentes a esses bancos terão suas autorizações negadas.
 
Ao Acessar a tela transacional, o comprador obtará pelo pagamento via Cartão de débito, e será redirecionado ao ambiente bancario para Autenticação e Autorização.
...
2. **Não Finalizado** - Status intermediario. Neste ponto o Checkout Cielo espera a confirmação do Banco sobre o status da autenticação e transação. Caso o comprador abandone o ambiente do banco, o status não se altera.
...
O Checkout Cielo permite a utilização de Débito Online (Transferencia entre contas bancarias) para compradores que possuam contas nos bancos Bradesco e Banco do Brasil.
Esse meio de pagamento é liberado via cadastro junto ao Suporte Cielo. 

Ao Acessar a tela transacional, o comprador obtará pelo pagamento via Débito online, e será redirecionado ao ambiente bancario para Autenticação e Autorização.
...
* **Não Finalizado** - Status intermediario. Neste ponto o Checkout Cielo espera a confirmação do Banco sobre o status da autenticação e transação. Caso o comprador abandone o ambiente do banco, o status não se altera.
...
|Tipo|Descrição|
|----|---------|
|`POST`| Notificação onde o lojista é passivo. Dois `POST HTTP` são disparados, um informando dados da venda e outro mudança de Status da transação|
...
**Caracteristicas das URLs**

Todas as 3 URLs devem possuir as seguintes caracteristicas:

* Devem ser URLs staticas
* Devem possuir menos de 255 caracteres
* Caracteres especiais não são suportatos
...
![Cadastro de URLS](http://fit-chair.cloudvent.net/Tutorial/public/images/Checkout/URLS.PNG)
## Notificação: POST
...
![Fluxo N.POST](http://fit-chair.cloudvent.net/Tutorial/public/images/Checkout/NPOST.png)


**Retorno aguardado para o envio da notificação:** `HttpStatus = 200 (OK)` - Post recebido e processado com sucesso

**IMPORTANTE** Se a `URL de Notificação` cadastrada retornar algum erro/estiver indisponivel, serão realizadas **3 novas tentativas, com intervalo de 1 hora entre cada POST*. 


Caso o POST não seja recebido, é possivel reenvia-lo manualmente, basta acessar o pedido em questão pelo Backoffice e clicar no Icone de envio:


![Reenvio de notificação](http://fit-chair.cloudvent.net/Tutorial/public/images/Checkout/Reenvipost.png)



Veja a descrição dos itens de notificação na sessão **"Conteudo do POST de NOTIFICAÇÃO"**
## Notificação: JSON

A notificação vai JSON é um metodo mais seguro e flexivel para o lojista de realizar uma consulta no Chekcout Cielo.
...
1. `POST de NOTIFICAÇÃO` - Ocorre quando a transação é finalizada. Esse  possui as Credenciais necessarias consultas transacionais.
...
![Fluxo N.JSON](http://fit-chair.cloudvent.net/Tutorial/public/images/Checkout/NJSON.png)



**Conteudo do POST de NOTIFICAÇÃO JSON:**
...
Tanto na Notificação via POST HTTP ou POST JSON, o conteudo dos dados retornados é o mesmo. 
Abaixo são descritos todos os campos retornados, assim como suas definições e tamanhos:


**Conteudo do POST de NOTIFICAÇÃO:**
...
O Checkout Cielo permite que o lojista realizae transações de crédito parceladas em até 12 vezes. 
...
* **Parcelamento via API** - O Lojista assume a responsabilidade do calculo das parcelas e envia ao Checkout como os valores do carrinho serão divididos.
...
Neste modo, o lojista controla o limite maximo de parcelas que a loja realizará pelo Backoffice Checkout. O Valor das parcelas é definido acessando a aba **Configurações** e alterando a sessão **Pagamentos**


![Seleção de Parcelas](http://fit-chair.cloudvent.net/Tutorial/public/images/Checkout/parcelamento.PNG)
...
* Todas as transações possuirão as mesmas opções de parcalamento.
...
Existem 2 tipos de parcelamento via API 

* Parcelamento Simplificado - Neste modelo, a loja apenas informa o numero de parcela disponivel para uma transação em especifico. 
* Parcelamento Controlado - Neste modelo, a loja envia o valor de cada parcela, controlando cada opção de pagamento disponivel.
### Parcelamento Simplificado


Nesta opção,  o Checkout realiza o cálculo das parcelas considerando valor total e quantidade de parcelas enviadas pelo lojista para apenas uma transação.

**ATENÇÃO:** Nesta opção de parcelamento, o numero de parcelas desejadas deve ser inferior a quantidade que está cadastrada no backoffice Checkout. 
...
O Parcelamento simplificado é realizando enviando o campo `MaxNumberOfInstallments` dentro do nó Payment. Isso forçará o Checkout a recalcular o valor do parcelamento.
...
| `Payment.RecurrentPayment.EndDate`   | YYYY-MM-DD   | Nгo         | 255     | Data onde a Recorrência se encerrará; Se não enviado a recorrencia se encerra somente se cancelada      |                                                              
...
Caso uma das transações da recorrencia não seja autorizada, o Checkout Cielo executa a retentativa automaticamente, o envio de uma nova transação, considerando:
...
**OBS**: Esse processo visa manter obter uma resposta positiva do processo de autorização, impedindo o lojista de perder a venda. O Processo de retentativa gerea pedidos duplicados dentro do Backoffice, pois o pedido original, negado, será apresentado na lista de Pedidos, junto com a nova transação autorizada 
...
<br>
![Cancelamento de recorrência](http://fit-chair.cloudvent.net/Tutorial/public/images/Checkout/PedidoRecCance.PNG)
...
* **Cancelar Recorrência** - Cancela o agendamento de futuras transações, encerrando a recorrência. Não cancela a transação atual nem as que já ocorreram. Essas necessitam ser canceladas manualamente.
...
* Sua afiliação Cielo deve ser habilitada para transacionar sem CVV ou Em recorrencia, do contrario, todas as transações recorrentes serão negadas.
