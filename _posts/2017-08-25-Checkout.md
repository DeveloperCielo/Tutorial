...
| `Options.ReturnUrl`           | Strin        | Condicional | 255     | Define para qual url o comprador será enviado após finalizar a compra.                                  |                                                                 |
...
> **Resultado da transação:** Para obter o retorno do status da transação, é necessario definir uma URL de NOTIFICAÇÃO. Veja a sessão de notificação para maiores informações.
...
| Campo s          | Descrição                                                                                                                                      | Tamanho Min. | Tamanho Máx. | Obrigatório |
|----- ------------|------------------------------------------------------------------------------------------------------------------------------------------------|--------------|--------------|-------------|
...
| Campo                              | Tipo         | Obrigatório | Tamanho | Descrição                                                                                               | Condicional                                                     |
|------------------------------------|--------------|-------------|---------|---------------------------------------------------------------------------------------------------------|-----------------------------------------------------------------|
| Payment.RecurrentPayment.Interval  | Alphanumeric | Sim         |         | Intervalo entre cada transação da recorrencia - Ver Recorrencia Programada                              |                                                                 |
...
Como a recorrência só existe na API, se o desenvolvedor mandar como Bem Físico, a **API obriga o envio do tipo de frete**. 
Se no contrato técnico existir o nó da recorrência, ele só pode mandar “Sem frete”, que não é um tipo válido para bens físicos (produtos). Sendo assim ele receberá a seguinte resposta:
...
| `Customer.Name`               |    OBS 3    |       34       |            60 (OBS,3)            |
| `Customer.Address.Street`     |    OBS 4    |   Street: 70   |          Ver comentário          |
| `Customer.Address.Number`     |    OBS 4    |   Number: 10   |          Ver comentário          |
| `Customer.Address.Complement` |    OBS 4    | Complement: 20 |          Ver comentário          |
| `Customer.Address.District`   |    OBS 4    |  District: 50  |          Ver comentário          |
...
| **OBS 2:** O valor é persistido no banco                      |
| **OBS 3:** A API Cielo trunca automaticamente                 |
| **OBS 4:** O valor é persistido na API Cielo                  |  

  
| Banco do Brasil                                                                                                 |
|:----------------------------------------------------------------------------------------------------------------|
|**OBS 1:** Não é enviado ao banco                                                                                    |
|**OBS 2:** Quando enviado acima de 9 posições, a API Cielo trunca automaticamente, considerando os últimos 9 dígitos |
|**OBS 3:** Caracteres válidos: <BR>**Letras de A a Z - MAIÚSCULAS** <BR> **Caracteres especiais**: <BR>hífen (-)<BR> apóstrofo (') <BR><BR> Quando utilizados, não pode conter espaços entre as letras; <BR><BR><BR> **Exemplos corretos**: D'EL-REI, D'ALCORTIVO, SANT'ANA.<BR><BR> **Exemplos incorretos**: D'EL - REI; até um espaço em branco entre palavras |
...
|:--------------------------------------------------------------|:----------------------------------------------------------------------------------------------------------------|
| **OBS 1:** Apenas Letras, números e caracteres como "_" e "$" |**OBS 1:** Não é enviado ao banco                                                                                    |
| **OBS 2:** O valor é persistido no banco                      |**OBS 2:** Quando enviado acima de 9 posições, a API Cielo trunca automaticamente, considerando os últimos 9 dígitos |
| **OBS 3:** A API Cielo trunca automaticamente                 |**OBS 3:** Caracteres válidos: <BR>**Letras de A a Z - MAIÚSCULAS** <BR> **Caracteres especiais**: <BR>hífen (-)<BR> apóstrofo (') <BR><BR> Quando utilizados, não pode conter espaços entre as letras; <BR><BR><BR> **Exemplos corretos**: D'EL-REI, D'ALCORTIVO, SANT'ANA.<BR><BR> **Exemplos incorretos**: D'EL - REI; até um espaço em branco entre palavras |
